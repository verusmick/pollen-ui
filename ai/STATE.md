...
- The review workflow is now peak-driven: the selected date range drives the chart query first, and validation-event images must only load after a peak/time slice is selected
- Do not preload validation-event images for the full date range; the validation-event request needs slice-scoped timestamps from the chart selection
- Current correction-factor detail payloads do not restore per-event reviewed classifications from stored multipliers
- Current correction-factor detail payloads also do not restore the originally reviewed peak/time slice
...
- Need confirmation whether selectable peak/time slices come from upstream metadata or should be derived client-side from measurements
